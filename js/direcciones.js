direccionesModulo = (function() {
  ...
  function calcularYMostrarRutas() {
    ...
    var points = [];
    var pointsIntermediate = document.getElementById('puntosIntermedios');
    for (var i = 0; i < pointsIntermediate.length; i++) {
      if (pointsIntermediate.options[i].selected) {
        points.push({
          location: pointsIntermediate[i].value,
          stopover: true
        });
      }
    }
